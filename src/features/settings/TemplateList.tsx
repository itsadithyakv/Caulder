          ...
          editing === template.id ? (
            ...
            <li key={template.id} className="templaterow">
              <div className="templaterow__body">
                <span className="templaterow__name">
                  {template.name}
                  {template.channel === "whatsapp" && (
                    <span className="badge badge--neutral">WhatsApp</span>
                  )}
                </span>
                <span className="templaterow__subject">
...
function TemplateForm({
  ...
  const [channel, setChannel] = useState(template?.channel === "whatsapp" ? "whatsapp" : "email");
  const [error, setError] = useState<string | null>(null);
  ...
  return (
    <form
      ...
      onSubmit={(event) => {
        ...
        // A WhatsApp message has no subject line, so the schema no longer
        // insists on one. An email without one is still a mistake, and that
        // is asserted here rather than in the schema, which now serves both.
        if (channel === "email" && subject.trim() === "") {
          setError("An email needs a subject line.");
          return;
        }
    ...
    >
      <div className="leadform__row">
        ...
        <div className="field">
          <label className="field__label" htmlFor="tpl-channel">
            Goes out by
          </label>
          <Chips
            value={channel}
