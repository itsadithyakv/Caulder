 ...
 *
 * The card body opens the deal's contact, where the deal is listed. Moving is a separate menu button, so a click
    ...
    <li className="boardcard" ref={wrapper}>
      <div
        ...
        onDragStart={(event) => {
          ...
          event.dataTransfer.setData("text/caulder-deal", card.id);
      ...
      >
        <button type="button" className="boardcard__open" onClick={onOpen}>
          <span className="boardcard__name">{card.title}</span>
          {(card.title !== card.name || card.contactPerson || card.city) && (
            <span className="boardcard__sub">
              {[card.title !== card.name ? card.name : null, card.contactPerson, card.city]
                .filter(Boolean)
                .join(" · ")}
            </span>
          )}
